...
generally means that pointers (namely strings) must live on aligned
addresses.
...
choice possible for a 64bit machine (and a pretty awful choice for a
32bit machine!)
...
bblocks in a file, etc. (This may effect performance depending on
where the allocator places these basic blocks because of cache
"spatial" locality.) These are garbage collected like any other C
structure since omni-c and the roci subsystem both use the bdw garbage
collector.
