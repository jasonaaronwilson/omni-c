...
int64_t get_file_modification_time(const char *filename) {
  ...
  // macOS uses st_mtimespec instead of st_mtim like linux.
  total_microseconds = (cast(int64_t, result.st_mtimespec.tv_sec) * 1000000LL) +
    (result.st_mtimespec.tv_nsec / 1000);
