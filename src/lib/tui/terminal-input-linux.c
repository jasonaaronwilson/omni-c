...
static term_modifier_t parse_modifiers(int mods) {
  ...
  if (mods & 8)
    m |= TERM_MOD_SUPER;
  if (mods & 16)
    m |= TERM_MOD_CAPS_LOCK;
  if (mods & 32)
    m |= TERM_MOD_NUM_LOCK;
  return m;
}

static int parse_sgr_mouse(const char* buf, size_t len,
                           term_input_event_t* ev) {
  int b = 0;
  int x = 0;
  int y = 0;
  char final_char = 0;
  ...
  if (b & 4)
    ev->mouse.modifiers |= TERM_MOD_SHIFT;
  if (b & 8)
    ev->mouse.modifiers |= TERM_MOD_ALT;
  if (b & 16)
    ev->mouse.modifiers |= TERM_MOD_CTRL;
  ...
  if (is_wheel) {
    if (btn_code == 0)
      ev->mouse.action = TERM_MOUSE_ACTION_WHEEL_UP;
    else if (btn_code == 1)
      ev->mouse.action = TERM_MOUSE_ACTION_WHEEL_DOWN;
    else if (btn_code == 2)
      ev->mouse.action = TERM_MOUSE_ACTION_WHEEL_LEFT;
    else if (btn_code == 3)
      ev->mouse.action = TERM_MOUSE_ACTION_WHEEL_RIGHT;
    ev->mouse.button = TERM_MOUSE_BTN_NONE;
  } else if (is_motion) {
    ev->mouse.action
        = (b & 3) ? TERM_MOUSE_ACTION_DRAG : TERM_MOUSE_ACTION_MOVE;
    ev->mouse.button = cast(term_mouse_button_t, (btn_code + 1));
  } else {
    ev->mouse.action = (final_char == 'M') ? TERM_MOUSE_ACTION_PRESS
...
static void map_kitty_pua_key(uint32_t codepoint, term_keypress_t* k) {
  ...
  // Navigation & editing
  case 57352:
    k->keycode = TERM_KEY_UP;
    break;
  case 57353:
    k->keycode = TERM_KEY_DOWN;
    break;
  case 57354:
    k->keycode = TERM_KEY_RIGHT;
    break;
  case 57355:
    k->keycode = TERM_KEY_LEFT;
    break;
  case 57356:
    k->keycode = TERM_KEY_PAGE_UP;
    break;
  case 57357:
    k->keycode = TERM_KEY_PAGE_DOWN;
    break;
  case 57358:
    k->keycode = TERM_KEY_HOME;
    break;
  case 57359:
    k->keycode = TERM_KEY_END;
    break;
  case 57360:
    k->keycode = TERM_KEY_INSERT;
    break;
  case 57361:
    k->keycode = TERM_KEY_DELETE;
    break;

  // Function keys
  case 57376:
    k->keycode = TERM_KEY_F1;
    break;
  case 57377:
    k->keycode = TERM_KEY_F2;
    break;
  case 57378:
    k->keycode = TERM_KEY_F3;
    break;
  case 57379:
    k->keycode = TERM_KEY_F4;
    break;
  case 57380:
    k->keycode = TERM_KEY_F5;
    break;
  case 57381:
    k->keycode = TERM_KEY_F6;
    break;
  case 57382:
    k->keycode = TERM_KEY_F7;
    break;
  case 57383:
    k->keycode = TERM_KEY_F8;
    break;
  case 57384:
    k->keycode = TERM_KEY_F9;
    break;
  case 57385:
    k->keycode = TERM_KEY_F10;
    break;
  case 57386:
    k->keycode = TERM_KEY_F11;
    break;
  case 57387:
    k->keycode = TERM_KEY_F12;
    break;

  // Standalone modifier reporting (Kitty flags 1 / 8)
  case 57441:
    k->keycode = TERM_KEY_SHIFT;
    k->location = TERM_KEY_LOC_LEFT;
    break;
  case 57442:
    k->keycode = TERM_KEY_CTRL;
    k->location = TERM_KEY_LOC_LEFT;
    break;
  case 57443:
    k->keycode = TERM_KEY_ALT;
    k->location = TERM_KEY_LOC_LEFT;
    break;
  case 57444:
    k->keycode = TERM_KEY_SUPER;
    k->location = TERM_KEY_LOC_LEFT;
    break;
  case 57447:
    k->keycode = TERM_KEY_SHIFT;
    k->location = TERM_KEY_LOC_RIGHT;
    break;
  case 57448:
    k->keycode = TERM_KEY_CTRL;
    k->location = TERM_KEY_LOC_RIGHT;
  ...
  // System / Locks
  case 57364:
    k->keycode = TERM_KEY_PRINT_SCREEN;
    break;
  case 57365:
    k->keycode = TERM_KEY_PAUSE;
    break;
  case 57367:
    k->keycode = TERM_KEY_CAPS_LOCK;
    break;
  case 57368:
    k->keycode = TERM_KEY_SCROLL_LOCK;
    break;
  case 57369:
    k->keycode = TERM_KEY_NUM_LOCK;
    break;
...
static int decode_utf8(const char* s, size_t len, uint32_t* out_cp) {
  ...
  if (c0 < 0x80) {
    *out_cp = c0;
    ...
    if (len < 2)
      return 0; // Incomplete
    *out_cp = ((c0 & 0x1F) << 6) | cast(unsigned char, s[1] & 0x3F);
    return 2;
  } else if ((c0 & 0xF0) == 0xE0) {
    if (len < 3)
      return 0;
    *out_cp = ((c0 & 0x0F) << 12) | (cast(unsigned char, s[1] & 0x3F) << 6)
              | (cast(unsigned char, s[2] & 0x3F));
    return 3;
  } else if ((c0 & 0xF8) == 0xF0) {
    if (len < 4)
      return 0;
    *out_cp = ((c0 & 0x07) << 18) | (cast(unsigned char, s[1] & 0x3F) << 12)
...
static int parse_csi_key(const char* buf, size_t len, term_input_event_t* ev) {
  ...
  if (term_idx >= len)
    return 0; // Sequence incomplete, wait for more data
  ...
  if (action == 1)
    ...
    ev->key.action = TERM_KEY_ACTION_REPEAT;
  else if (action == 3)
  ...
  if (final_char == 'u') {
    ...
    switch (final_char) {
    case 'A':
      ev->key.keycode = TERM_KEY_UP;
      break;
    case 'B':
      ev->key.keycode = TERM_KEY_DOWN;
      break;
    case 'C':
      ev->key.keycode = TERM_KEY_RIGHT;
      break;
    case 'D':
      ev->key.keycode = TERM_KEY_LEFT;
      break;
    case 'H':
      ev->key.keycode = TERM_KEY_HOME;
      break;
    case 'F':
      ev->key.keycode = TERM_KEY_END;
      break;
    case 'Z':
      ev->key.keycode = TERM_KEY_TAB;
      ev->key.modifiers |= TERM_MOD_SHIFT;
      break;

      // F1 - F4 CSI letter variants (\x1b[P, \x1b[1;2Q, etc.)
    case 'P':
      ev->key.keycode = TERM_KEY_F1;
      break;
    case 'Q':
      ev->key.keycode = TERM_KEY_F2;
      break;
    case 'R':
      ev->key.keycode = TERM_KEY_F3;
      break;
    case 'S':
      ev->key.keycode = TERM_KEY_F4;
      break;
...
int term_poll_event(term_input_event_t* ev) {
  ...
  if (in_len < sizeof(in_buf) - 1) {
    ssize_t n
        = read(STDIN_FILENO, in_buf + in_len, sizeof(in_buf) - 1 - in_len);
    if (n > 0) {
      in_len += n;
      in_buf[in_len] = '\0';
    }
  ...
  if (in_buf[0] == '\x1b') {
    ...
    if (in_len >= 3 && in_buf[1] == '[' && in_buf[2] == '<') {
      ...
      if (consumed > 0)
        goto event_ready;
      if (in_len < 32)
        return 0; // Wait for terminating M/m
    }
    ...
    if (in_len >= 2 && in_buf[1] == '[') {
      ...
      if (consumed > 0)
        goto event_ready;
      if (in_len < 32)
        return 0; // Wait for full sequence
    }
  ...
  // 3. UTF-8 / Standard ASCII Fallback
  {
    ...
    if (consumed == 0)
      return 0; // Partial UTF-8 multibyte sequence, wait for next tick
    ...
    // Legacy Control Characters: Ctrl+A (1) to Ctrl+Z (26), excluding Tab (9) &
    // Enter (10/13)
