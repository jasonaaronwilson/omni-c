...
char* quote_c_string(char* input) {
  ...
  for (size_t i = 0; input[i] != 0; i++) {
    ...
    case '\r':
      ...
      break;
      // case '\b': buffer_append_byte(buf, '\\'); buffer_append_byte(buf, 'b');
      // break; case '\f': buffer_append_byte(buf, '\\');
      // buffer_append_byte(buf, 'f');  break;
    default:
      if (c >= 32 && c < 126) {
  ...
  for (size_t i = 1; i < limit;) {
    ...
    if (c == '\\') {
      ...
      case 'r':
        ...
        break;
      // case 'b': buffer_append_byte(buf, '\\'); buffer_append_byte(buf, 'b');
      // break; case 'f': buffer_append_byte(buf, '\\'); buffer_append_byte(buf,
      // 'f');  break;
      case 'x': {
