...
codepoint_array_t* cp_array_make(void) {
  codepoint_array_t* arr
      = cast(codepoint_array_t*, malloc_bytes(sizeof(codepoint_array_t)));
  arr->capacity = 32;
  arr->length = 0;
...
void cp_array_insert(codepoint_array_t* arr, size_t index, uint32_t cp) {
  ...
  if (index < arr->length) {
    memmove(&arr->chars[index + 1], &arr->chars[index],
            (arr->length - index) * sizeof(uint32_t));
  }
  arr->chars[index] = cp;
  arr->length++;
}
...
void cp_array_delete(codepoint_array_t* arr, size_t index) {
  ...
  if (index + 1 < arr->length) {
    memmove(&arr->chars[index], &arr->chars[index + 1],
            (arr->length - index - 1) * sizeof(uint32_t));
  }
  arr->length--;
}
